...
public class ProductServiceImpl implements ProductService
{
    ...
    @Override
    public <T> List<T> getAllBySearchPaginated(SearchDto dto, int pageNumber, Class<T> type)
    {
        this.validateSearchDto(dto);
        final String categoryIds = convertIdsToString(dto.getCategoryIds());
        final String[] searchTerms = dto.getTerm() != null ? dto.getTerm().split(" ") : null;
        ...
        final List<Product> products = this.productDao.findAllByFiltersPaginated(searchTerms, categoryIds, dto.getMinPrice(), dto.getMaxPrice(), firstResult, PAGE_SIZE);
        return products.stream()
                       .map(source -> this.mapper.map(source, type))//
                       .collect(Collectors.toList());
    }
